                        ...
                        <Grid x:Name="DragBar"
                            ...
                            <StackPanel Orientation="Horizontal" VerticalAlignment="Center">
                                <Border Width="22"
                                        ...
                                        Margin="0,0,10,0">
                                    <Grid>
                                        <Image x:Name="ImgAppIcon" Stretch="Uniform" Margin="2"/>
                                        <TextBlock x:Name="TxtIconFallback"
                                                   Text="A"
                                                   Foreground="White"
                                                   FontWeight="Bold"
...
<ResourceDictionary xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <SolidColorBrush x:Key="ScrollTrackBrush" Color="#0F1826"/>
    <SolidColorBrush x:Key="ScrollThumbBrush" Color="#25A7C2"/>
    <SolidColorBrush x:Key="ScrollThumbHoverBrush" Color="#43C6E0"/>
    <SolidColorBrush x:Key="ScrollThumbPressedBrush" Color="#1B7E95"/>
    ...
    <Style TargetType="{x:Type TextBox}" BasedOn="{StaticResource AppTextBoxStyle}"/>
...
$controls = @{}
foreach ($name in @("RootShellBorder", "DragBar", "BtnMin", "BtnMax", "BtnClose", "TxtMasterPrompt", "MasterPromptScroll", "ImgAppIcon", "TxtIconFallback")) {
...
if ($iconPath) {
    ...
    if ($controls.ImgAppIcon) {
        try {
            $iconUri = New-Object System.Uri($iconPath)
            $decoder = New-Object System.Windows.Media.Imaging.IconBitmapDecoder($iconUri, [System.Windows.Media.Imaging.BitmapCreateOptions]::None, [System.Windows.Media.Imaging.BitmapCacheOption]::OnLoad)
            if ($decoder.Frames.Count -gt 0) {
                $controls.ImgAppIcon.Source = $decoder.Frames[0]
                if ($controls.TxtIconFallback) { $controls.TxtIconFallback.Visibility = [System.Windows.Visibility]::Collapsed }
            }
        } catch {
        }
    }
